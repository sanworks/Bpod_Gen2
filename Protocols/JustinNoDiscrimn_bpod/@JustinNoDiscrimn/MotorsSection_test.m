...
switch action
    ...
    case 'init',   % ------------ CASE INIT ----------------
        ...
        active_locations.coords = [0 0];
        active_ids=1:1:numel(active_locations);
        ...
        for g=1:1:numel(active_ids)
            ...
            active_locations.handle_name{g} = 'handle';
        ...
        SoloParamHandle(obj,'current_location_index','name','','value',1);
    ...
    case 'update_active_locs'
        ...
        active_locations.coords=[axialActive_temp(:) radialActive_temp'];
        active_ids=1:1:length(active_locations.coords);
        
        %these are essentially default settings. Can reset this later
        %through a button.
        for g=1:1:numel(active_ids)
            active_locations.axial{g}=active_locations.coords(g,1);
            active_locations.radial{g}=active_locations.coords(g,2);
        end
    ...
    case 'move_next_side', % --------- CASE MOVE_NEXT_SIDE -----
        ...
        switch SessionType
            ...
            otherwise %we are using motors
                ...
                t=0;
                while t==0
                    prePoint=active_locations.coords(randsample(length(active_locations.coords),1),:);

                    if sum(active_locations.coords(ind,:)-prePoint)==0
                        t=0;
                    else
                        t=1;
                    end
                ...
                set_history(currentPolePosition,[value(currentPolePosition);...
                    [active_locations.coords(ind,1)*10000 active_locations.coords(ind,2)*10000]]);
                
                currentPolePosition.value = [value(currentPolePosition); active_locations.coords(ind,1)*10000 active_locations.coords(ind,2)*10000];
                ...
                move_absolute_mult(pole_motors,[active_locations.coords(ind,2)*10000 active_locations.coords(ind,1)*10000], ...
                    [pole_motors_properties.radial_motor_num pole_motors_properties.axial_motor_num]);
