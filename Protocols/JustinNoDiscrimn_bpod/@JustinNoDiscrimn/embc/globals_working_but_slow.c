...
void tick_func(void){
	...
	* by a touch, to get reward
	...
	if (curr_state==64 || curr_state==65 || curr_state==66 || curr_state==67 || curr_state==68 || curr_state==69 || curr_state==70 || curr_state==71 || curr_state==72){ /*NOT SURE WHY I need to do this, but maske sure to sensor threshold detection only in the states we want to detect it*/
		if (mean1 >= 0.01)  { 
			v_state_up = 1;
			touch_state_flag = 1;
			if (touchblerg == 0) {
	...
	if (curr_state >= 40 && curr_state <= 77) {
		if  (v_state_trans > 0 ) {	
	...
	if (curr_state == 64 || curr_state==65 || curr_state==66 || curr_state==67){ /*NOT SURE WHY I need to do this, but maske sure to sensor threshold detection only in the states we want to detect it*/
	...
	/*RESPONSE STATES FLAGGING, e.g. actions during delay and sample period*/
	/*NOT SURE WHY, BUT THE FOLLOWING ONLY MAKES SENSE IF WE USE 'ANSWER_MODE' VAR. WTF*/
	resp_flag=0;
	if (curr_state == 64 || curr_state == 65) { 
		if (strcmp(answer_mode,"lick")!=0) {  /*animal must lick only to respondd. Reward depends on trial type*/
			if (lick_state_flag==1) {
				resp_flag=1;
			} 
		}else if (strcmp(answer_mode,"touch")!=0) {  /*animal must do correct touch response*/
			if (touch_state_flag == 1) {
				resp_flag=1;
			} 
		}else if (strcmp(answer_mode,"whiskvel")!=0) {  /*animal must do whisk respinse, then touch response,  +lick to get reward*/
	...
	if (curr_state == 66 || curr_state == 67) { 
		...
		if (whisk_ans_time - cycle_counter == 0 || touch_ans_time - cycle_counter == 0 || lick_ans_time - cycle_counter == 0) {
			first_answer_time=cycle_counter;
			ans_flag=1;
		}
	}
	...
	if (curr_state == 66) {
		if ((cycle_counter - first_response_time) >= answer_delay) { /*answer delay period has passed*/
			triggerSound(0,rew_cue); /*trigger a delay period over cue*/
	...
	if (curr_state >=40 && curr_state <= 77) {
		...
		if (log_touch_counter == 24) { /*(12 cycles is every 2ms, or 500HZ, at 6kHz**/ 
			/*logValue("absdiff1", absdiff1);*/
			/*logValue("touch1", mean1);	
			/*logValue("rawtouch1", v1);*/	
			log_touch_counter = 0;
		
		}
		if (log_whisk_counter == 80) { /*(38 cycles is every ~6ms, or ~120HZ, at 6kHz*, */ 
