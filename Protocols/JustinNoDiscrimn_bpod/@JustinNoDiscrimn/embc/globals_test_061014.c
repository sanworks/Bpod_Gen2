...
/*const int numCycThresh = XXX;
/*const int numCycVal = XXX;
...
/* 	if (cycCounter > numCycThresh + numCycVal) {
/* 		for (i = 0; i <= numCycThresh + numCycVal - 2; i++) { /*Step back through the buffer replacing old elements until the last element is reached*/
/* 			vVect1[i] = vVect1[i+1];
/* 			vVect2[i] = vVect2[i+1];
/*  		}
		
/* 		vVect1[sizeof(vVect1) - 1] = v1; /*last number is now the latest v measure*/
/* 		vVect2[sizeof(vVect1) - 1] = v2; /*last number is now the latest v measure*/
/* 	}
...
/*			mean1 = mean1 * -1; /*want this to be positive for when we add to sensor 2*/
/*		}
