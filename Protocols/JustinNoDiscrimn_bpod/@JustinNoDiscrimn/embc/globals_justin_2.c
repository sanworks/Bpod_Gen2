...
const unsigned whisker_detector_ai_chan = 8; /* Analog input channel for whisker position readings. */
const unsigned aom_473_chan = 9; /* Analog output channel for AOM for 473 nm laser. */
const unsigned aom_594_chan = 10; /* Analog output channel for AOM for 594 nm laser. */
...
void tick_func(void)
{
 ...
 /*   unsigned num_log_items;
     ...
     *it in the queue. */
        if (v_state == 0 && v_state_last == 1) { /* Whisker touch is over */
