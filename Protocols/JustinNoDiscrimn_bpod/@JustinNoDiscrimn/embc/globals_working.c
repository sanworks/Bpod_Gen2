...
int bypassChanState_touchled = 0;  
...
void tick_func(void){
	...
	if (curr_state > 43 && curr_state <= 45){ 
		if (mean1 >= touch_thresh)  {
			bypassChanState_touchled = 1; /*trigger touch on*/
			v_state_up = 1;
			...
			bypassChanState_touchled = 0; /*trigger touch on*/
			v_state = 0;
			v_state_up = 0;
			v_state_down = 0;
			touch_state_flag = 0;
	...
	/*make sure delay from pole cue to go cue is the length of the delay period*/
	/*also make sure that for at least once cycle we are 'officially' in the delay state*/
	/*if ((delay_period - resonance_delay) <= 0) {
	/*	delay_period = 1;
	/*}else{
	/*	delay_period = (delay_period  - resonance_delay); 
	/*}
	...
	*/
	...
	if (bypassChanState_touchled == 1){
		bitmask=bitmask+bypassChanIds_touchled;
	}
