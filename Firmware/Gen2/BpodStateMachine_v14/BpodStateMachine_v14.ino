...
*/
// Bpod State Machine Firmware Ver. 14
//
// SYSTEM SETUP:
//
...
// The second row lists the physical input and output channels on Arduino for B,W,P, and the SPI CS pin is listed for S. Use 0 for UART and USB.
...
#if MachineType == 1 // Arduino Due-based Bpod state machine v0.5+
  #if FirmwareVersion > 7 // Bpod State Machine r0.7+
  ...
  #endif
#else // Teensy 3.6-based "Pocket" state machine
...
byte GlobalTimerLoop[nGlobalTimers] = {0}; // true if timer restarts automatically until trial end or until explicitly stopped
  ...
  if (CurrentTimer > 0) {
    for (int i = 0; i < nGlobalTimers; i++) {
      if (bitRead(CurrentTimer, i)) {
        GlobalTimersTriggered[i] = true;
        GlobalTimerStart[i] = CurrentTime;
        if (GlobalTimerOnsetDelays[i] > 0){
          GlobalTimerStart[i] += GlobalTimerOnsetDelays[i];
        } else {
          GlobalTimersActive[i] = true;
          setGlobalTimerChannel(i, 1);
        }
