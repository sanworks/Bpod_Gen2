  ...
  if (BpodCOM.available() > 0) { // If a message has arrived on the USB serial port
    ...
    switch (CommandByte) {
      ...
      case 'T': // Receive bytes from USB and send to hardware serial channel 1-5
        Byte1 = BpodCOM.readByte() - 1; // Serial channel
        nBytes = BpodCOM.readUint32();
        for (int i = 0; i < nBytes; i++) {
          switch (Byte1) {
            case 0:
              Serial1COM.writeByte(BpodCOM.readByte());
            break;
            case 1:
              Serial2COM.writeByte(BpodCOM.readByte());
            break;
            case 2:
              Serial3COM.writeByte(BpodCOM.readByte());
            break;
            #if MachineType == 2
              case 3:
                Serial4COM.writeByte(BpodCOM.readByte());
              break;
              case 4:
                Serial5COM.writeByte(BpodCOM.readByte());
              break;
            #endif
          }
        }
      ...
      case 'U': // Recieve byte CODE from USB and send to hardware serial channel 1-5
        Byte1 = BpodCOM.readByte() - 1; // Serial channel
        Byte2 = BpodCOM.readByte(); // Byte CODE
