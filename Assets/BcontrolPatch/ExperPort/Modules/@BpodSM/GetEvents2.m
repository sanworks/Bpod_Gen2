...
function [eventList] = GetEvents2(sm, start_no, end_no)
    if end_no == 0 %start_no > end_no, % Changed to force GET EVENTS_II each time data is available
        eventList = zeros(0,4);
    else
        %eventList = DoQueryMatrixCmd(sm, sprintf('GET EVENTS_II %d %d', start_no-1, end_no-1));
        ok = sm.handle.sendstring('GET EVENTS_II', start_no, end_no);
        eventList = sm.handle.readmatrix;
        k = 5;
    end;
