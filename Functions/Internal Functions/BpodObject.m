...
classdef BpodObject < handle
    ...
    methods
        function obj = BpodObject %Constructor
            ...
            if isfield(obj.SystemSettings, 'SoloProtocolFolder')
                if exist(obj.SystemSettings.SoloProtocolFolder)
                    obj.Path.SoloProtocolFolder = obj.SystemSettings.SoloProtocolFolder;
                end
            end
            
            %JPL - bit hackish, but works for now
            %look in settings dir for settings files produced by
            %protocolTranslation
            t=dir(obj.Path.SettingsDir);
            bcDirs={t(find(cellfun(@(x) ~isempty(strfind(x,'_bpod_settings.mat')),{t.name}))).name};
            for k=1:1:numel(bcDirs)
               load([obj.Path.SettingsDir filesep bcDirs{k}])
               prots{k}=[S.translationObject.soloDir filesep 'Protocols'];
            end
            ...
            if ~exist(obj.Path.OutputConfig)
                copyfile(fullfile(obj.Path.BpodRoot, 'Examples', 'Example Settings Files', 'OutputConfig.mat'), obj.Path.OutputConfig);
            end
            load(obj.Path.OutputConfig);
        ...
        %on a module
        %putting some temporary values in here for now
        
        function [strings] = getInputChansAndPinsForModule(varargin)
            moduleName=varargin{2};
            switch moduleName
                
                case 'AnalogIn1'
                    strings={'AI1','AI2','AI3','AI4','AI5','AI6','AI7','AI8','DI1','DI2',}; %analog input module
                case 'WavePlayer1'
                    strings={'Serial1'}; %analog output module, 4-chan
                case 'AnalogOut8' %JPL - dont actually know what this is called
                    strings={'Serial1'}; %analog output module, 8-chan
                case 'DDS'
                     strings={''};
                case 'SNES'
                    strings={''};
                case 'Serial1'  %JPL - I think these appear as modules when the serial streaming is plugged in?
                    strings={'Serial1'};
                case 'Serial2'
                    strings={'Serial1'};
                case 'Serial3'
                ...
                otherwise
                    strings={''};
                    %warning(['BpodObject::getInputChansAndPinsForModule - dont recognize module: ' moduleName])
            end
        end
        
        function [strings] = getOutputChansAndPinsForModule(varargin)
            moduleName=varargin{2};
            switch moduleName
                
                case 'AnalogIn1'
                    strings={'DO1','DO2',}; %analog input module
                case 'WavePlayer1'
                    strings={'AO1','AO2','AO3','AO4'}; %analog output module, 4-chan
                case 'AnalogOut8' %JPL - dont actually know what this is called
                    strings={'AO1','AO2','AO3','AO4','AO5','AO6','AO7','AO8'}; %analog output module, 8-chan
                case 'DDS'
                     strings={''};
                case 'SNES'
                     strings={''};
                ...
                otherwise
                    strings={''};
                    %warning(['BpodObject::getOutChansAndPinsForModule - dont recognize module: ' moduleName])
