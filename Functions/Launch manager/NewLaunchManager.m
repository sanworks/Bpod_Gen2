...
<<<<<<< HEAD
    ...
    for x = 1:length(Candidates)
        if Candidates(x).isdir && isempty(strfind(Candidates(x).name,'.'))
            ProtocolFolder = fullfile(BpodSystem.Path.SoloProtocolFolder, Candidates(x).name);
            Contents = dir(ProtocolFolder);
            nItems = length(Contents);
            Found = 0;
            for y = 1:nItems
                %JPL
                if ~isempty(strfind(Candidates(x).name,'@'))
                    tmpname=Candidates(x).name(2:end);
                else
                    tmpname=Candidates(x).name;
                end
                if strcmp(Contents(y).name, [tmpname '.m'])
                    Found = 1;
                end
            end
            if Found
                sProtocolName = Candidates(x).name;
            else
                sProtocolName = ['<' Candidates(x).name '>'];
            end
            nProtocols = nProtocols + 1;
            ProtocolNames{nProtocols} = sProtocolName;
        end
    end
...
if ~isempty(strfind(ProtocolName,'@')) %solo style protocol
    load([BpodSystem.Path.SettingsDir filesep [ProtocolName(2:end) '_settings.mat']])
    BpodSystem.ProtocolTranslation=S.translationObject;
    clear S;
    ProtocolFileName = fullfile([BpodSystem.ProtocolTranslation.soloDir filesep 'Protocols' filesep, [ProtocolName(2:end) '_wrapper.m']]);
    ProtocolPath = fullfile([BpodSystem.ProtocolTranslation.soloDir filesep 'Protocols' filesep, [ProtocolName(2:end) '_wrapper.m']]);
else
