...
if BpodLib.multi.isMultiSetup(BpodSystem)
    ...
    choice = questdlg(sprintf(['The Bpod State Machine COM port (%s) does not match the last port used for liquid calibration (%s).\n\n', ...
        'Would you like to update the existing calibration to expect %s?'],... 
        BpodLib.utils.getCurrentCOM(BpodSystem), LiquidCal.metadata.COM, BpodLib.utils.getCurrentCOM(BpodSystem)),...
        'Liquid calibration COM Port Mismatched', ...
        'Update existing calibration', 'I will remember to re-calibrate', ...
    ...
    if strcmp(choice, 'Update existing calibration')
