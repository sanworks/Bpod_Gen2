        ...
        function obj = LiquidCalibratorUI(varargin)
                ...
                case 'PortArray'
                    ...
                    for i = 1:obj.BpodSystem.Modules.nModules
                        if ~isempty(strfind(obj.BpodSystem.Modules.Name{i}, 'PA'))
                            nValves = nValves + length(obj.BpodSystem.Modules.EventNames{i})/2;
                        end
                    end
            end
            ...
            if arrayConnected || strcmp(obj.source, 'portarray')
                if ~isempty(obj.BpodSystem.CalibrationTables.PortArrays)
                    % Check to ensure that physical setup matches existing calibration setup
                    PAchannels = zeros(1,obj.BpodSystem.Modules.nModules);
                    for i = 1:obj.BpodSystem.Modules.nModules
                        if ~isempty(strfind(obj.BpodSystem.Modules.Name{i}, 'PA'))
                            PAchannels(i) = length(obj.BpodSystem.Modules.EventNames{i})/2;
                        end
                    end
                    PAchannels = PAchannels(PAchannels~= 0);
                    CalChannels = [];
                    valveNames = fieldnames(obj.BpodSystem.CalibrationTables.PortArrays.ValveDatas);
                    portCount = 0;
                    for i = 1:length(valveNames)
                        thisValveName = valveNames{i};
                        if str2double(thisValveName(5)) < portCount
                            CalChannels = [CalChannels portCount];
                            portCount = 1;
                        else
                            portCount = portCount + 1;
                        end
                        if i == length(valveNames)
                            CalChannels = [CalChannels portCount];
                        end
                    end

                    paSetupChanged = false;
                    if length(CalChannels) ~= length(PAchannels)
                        paSetupChanged = true;
                    elseif sum(CalChannels == PAchannels) ~= length(CalChannels)
                        paSetupChanged = true;
                    end

                    errormsg = 'Port Array setup has changed since last calibration. Delete /Bpod Local/Config/LiquidCalibration-PortArrays.json and restart Bpod.';
                    if paSetupChanged
                        errordlg(errormsg)
                        error(errormsg);
                    end
