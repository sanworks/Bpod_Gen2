...
switch action,
  ...
  case 'init'
	...
	while not_connected
		try
			...
			elseif fake_rp_box==4,
				state_machine.value = BpodSM(state_machine_server);			
            else
    ...
    if     fake_rp_box==2 || fake_rp_box==20,   
        sound_machine.value = RTLSoundMachine(sound_machine_server); % <~> added 20, 2008.June.25
    ...
    elseif fake_rp_box==4
        sound_machine.value = BpodSoundServer(sound_machine_server); % 
    else
