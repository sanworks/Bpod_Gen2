...
switch action,
  ...
  case 'init', 
    ...
    %JPL - is this necessary?
    if ~isempty(BpodSystem)
        prot_list{1}='';
        prot_list{2}=BpodSystem.ProtocolTranslation.protocolName;
    end
  ...
  case 'rescan_protocols',    
    ...
    while ~strcmp(get(f, 'Type'), 'figure') && f~=0
        f = get(f, 'Parent'); 
    end;
  ...
  case 'scan_protocols',
    ...
    if isempty(pdirs) || (isscalar(pdirs) && isnan(pdirs))
        pdirs = './Protocols'; 
    end;
    pdirs = textscan(pdirs, '%s', 'Delimiter', ':'); pdirs = pdirs{1};
    prot_names = [];
    %JPL - some mods here to handle a return of no protocols
    for i=1:numel(pdirs),
      if exist(pdirs{i}, 'dir') && iscell(pdirs)
        prot_names = [prot_names ; dir([pdirs{i} filesep '@*'])]; %#ok<AGROW>
      end;
    end;
    
    if isstruct(prot_names)
        [prot_list{1:length(prot_names)}]= deal(prot_names.name);
        % remove @ head:
        for i=1:length(prot_list)
            prot_list{i} = prot_list{i}(2:end);
        end;
    else
        prot_list{1}='';
    end
    ...
    for i=1:length(prot_mfiles)
        prot_mfiles{i} = [prot_mfiles{i}(1:end-2) 'obj'];
    end;
