...
switch action,
        ...
        if ispc
            switch x,
             case 'start',
                ...
                evalstr=sprintf('ssh %s "%s %d %s %s %s &"',vid_ip,evalcmd,vid_num,experimenter,ratname,suffix);
             case 'stop',
                   %  this_dir=cd;
                  %   cd('\ratter\Rigscripts');
                evalstr=sprintf('\\ratter\\Rigscripts\\VCStopRecord.bat %s %d %s %s %s &',vid_ip,vid_num,experimenter,ratname,suffix);   
          %   cd(this_dir);
            end 
        else
            switch x,
         ...
         evalstr=sprintf('ssh -f brodylab@%s "%s %d %s %s %s"',vid_ip,evalcmd,vid_num,experimenter,ratname,suffix);
        end
        system(evalstr);
        catch me
  ...
  case 'deal_with_remaining_events',    
    ...
    if in_iti_states_flag==0, %#ok<NODEF> (defined by GetSoloFunctionArgs)
      ...
      %JPL - For bcontrol running through Bpod, we do not enforce the use
      %of a state0, so u is always empty. 
      
      if isempty(u) && isempty(BpodSystem)
      ...
      elseif ~isempty(BpodSystem)
        %prepare events from Bpod
        %the 'RawEvents' field of the translation objects is the output of
        %the bpod function 'RunStateMatrix', which is called in 'send.m'
         events=BpodSystem.ProtocolTranslation.BpodEvents;
