...
if ismember(n_s, [8 9 10 11]),
    if min_server(sm, 14, 'SetDIOSchedWaveSpecLength')
...
if isnumeric(use_happenings) && use_happenings==0,
    if min_server(sm, 14)
        DoSimpleCmd(sm, 'DO NOT USE HAPPENINGS'); 
    end;
else
    if min_server(sm, 14),
        DoSimpleCmd(sm, 'USE HAPPENINGS');
    else
        error('BadVersion', 'Cannot use happenings unless the server version >= 14');
    end;
end;
...
if ~isnumeric(use_happenings) || ~(use_happenings==0), % More recent servers will be expecting happening specs and a happening list
    ...
    if ~ok,
        error('BpodSM:BadCommmunication', 'happening spec couldn''t be sent');
    end;
    ...
    if ~ok,
        error('BpodSM:BadCommmunication', 'happening list couldn''t be sent');
    end;
end;
...
%JPL 
sma_bpod=BCtoBpod(sm,BpodSystem.ProtocolTranslation.inSMA);

SendStateMatrix(sma_bpod);
