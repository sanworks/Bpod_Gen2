...
if ~isstruct(spec) || ~isempty(setdiff(reqfields, fields(spec))),
    error('BpodSM:BadSyntax', 'spec must be a structure with fields %s', reqfields);
end;

res=1;
%res = FSMClient('sendstring', sm.handle, sprintf('SET HAPPENINGS SPEC\n'));
if res ~= 1, error('BpodSM:BadConnection', 'Couldn''t communicate with the FSM Server'); end;
