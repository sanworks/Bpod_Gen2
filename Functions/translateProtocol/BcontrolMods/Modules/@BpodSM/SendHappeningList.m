...
if ~min_server(sm, 14, mfilename),
    ok=0;
    return;
end;

if ~iscell(happList) || size(happList,2) ~= 1,
    error('BpodSM:BadSyntax', 'happList must be a cell with only one column');
end;

totalHappenings = 0;
for i=1:numel(happList)
    if size(happList{i},1)>1 || rem(numel(happList{i}), 2) ~= 0,
        error('BpodSM:BadSyntax', ['each row of happList must be a cell with a single row, ' ...
    ...
    for j=1:2:numel(happList{i})
        if ~ischar(happList{i}{j}) || ~isnumeric(happList{i}{j+1}),
            error('BpodSM:BadSyntax', ['odd-numbered entries in rows of happList must be ' ...
...
if res ~= 1, error('BpodSM:BadConnection', 'Couldn''t communicate with the Bpod'); end;
